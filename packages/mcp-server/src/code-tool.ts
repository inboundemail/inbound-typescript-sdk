...
import { ClientOptions } from 'inboundemail';

const prompt = `Runs JavaScript code to interact with the Inbound API.
  ...
  const res = await fetch(codeModeEndpoint, {
    ...
    headers: {
      ...
      'x-stainless-mcp-client-envs': JSON.stringify({
        INBOUND_API_KEY: requireValue(
          readEnv('INBOUND_API_KEY') ?? client.apiKey,
          'set INBOUND_API_KEY environment variable or provide apiKey client option',
        ),
        INBOUND_BASE_URL: readEnv('INBOUND_BASE_URL') ?? client.baseURL ?? undefined,
      }),
    },
    body: JSON.stringify({
