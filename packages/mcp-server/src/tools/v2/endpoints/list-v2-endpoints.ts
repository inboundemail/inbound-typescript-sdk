  ...
  inputSchema: {
    ...
    properties: {
      ...
      sortBy: {
        ...
        enum: ['newest', 'oldest', 'undefined'],
      },
      type: {
        type: 'string',
        description: 'type parameter',
        enum: ['webhook', 'email', 'email_group', 'undefined'],
      },
      jq_filter: {
        type: 'string',
        title: 'jq Filter',
...
export const handler = async (client: Inbound, args: Record<string, unknown> | undefined) => {
  ...
  return asTextContentResult(await maybeFilter(jq_filter, await client.v2.endpoints.list(body)));
};
