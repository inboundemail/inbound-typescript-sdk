...
export const handler = async (client: Inbound, args: Record<string, unknown> | undefined) => {
  ...
  try {
    return asTextContentResult(await maybeFilter(jq_filter, await client.emailAddresses.update(id, body)));
  } catch (error) {
