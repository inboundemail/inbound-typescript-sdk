...
import { Inbound, ClientOptions } from 'inboundemail';
...
function getTSDiagnostics(code: string): string[] {
  ...
  const codeWithImport = [
    ...
    functionSource.type === 'declaration' ?
      `async function run(${functionSource.client}: Inbound)`
    : `const run: (${functionSource.client}: Inbound) => Promise<unknown> =`,
    functionSource.code,
  ].join('\n');
...
const fuse = new Fuse(
  [
    ...
    'client.emailAddresses.delete',
    'client.emailAddresses.list',
    'client.emailAddresses.retrieve',
